...
def get_edge_label(layer):
    ...
    if layer.type == 'Data' or layer.type == 'MemoryData':
...
#        if dummy_pattern.match(node.get_name()) or memory_pattern.match(node.get_name()):
#	    continue
#	if node.get_name() == 'states' or node.get_name() == 'actions':
#		node.set_shape('ellipse')
#		node.set_fillcolor('white')
#	
#	print(node.get_name(), node.get_fillcolor())
        pydot_graph.add_node(node)

#    for i in range(len(pydot_edges)-1):
#        if pydot_edges[i]['src'] == pydot_edges[i+1]['dst'] and pydot_edges[i+1]['src'] == pydot_edges[i]['dst']:
#            print('yes ', pydot_edges[i]['src'])


    for edge in pydot_edges:
        src_name = pydot_nodes[edge['src']].get_name()
	dst_name = pydot_nodes[edge['dst']].get_name()
#        if dummy_pattern.match(src_name) or dummy_pattern.match(dst_name) or memory_pattern.match(src_name) or memory_pattern.match(dst_name):
#	    continue
