 ...
 * input to a EuclideanWSLossLayer exactly formulates a linear least squares
...
class EuclideanWSLossLayer : public LossLayer<Dtype> {
 public:
  explicit EuclideanWSLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "EuclideanWSLoss"; }
  /**
   * Unlike most loss layers, in the EuclideanWSLossLayer we can backpropagate
 ...
 protected:
   ...
   *
   * Unlike other children of LossLayer, EuclideanWSLossLayer \b can compute
